...
class StellaratorOpenMCBlanketSimulation(OpenMCBlanketSimulation):
    ...
    def _create_3d_importance_map(self, data_dict : Dict):

        from .sn.base_sn import create_3D_importance_map_of_blanket
        create_3D_importance_map_of_blanket()
    

    def create_fw_cadis_weight_window()
...
def _generate_flux_surface_source(flux_surface : ParametrisedSurface, s_spacing : jnp.ndarray, source_kwargs : Dict, n_theta : int, n_phi : int, toroidal_extent : ToroidalExtent, source_mesh_filename : str):
        ...
        openmc_mesh_sources            = onp.frompyfunc(make_source, 1, 1)(reaction_rates_volume_weighted)        
        return openmc.MeshSource(umesh_sourcee, openmc_mesh_sources)